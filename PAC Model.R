...
#xgboost (one-hot hand encoding for region)
...
pred_test_xgboost = predict(xgboost, 
                    newdata=as.matrix(test_input))

rmse_test_xgboost = sqrt(mean((pred_test - test$price)^2)) #26.12953 RMSE train



pred_trian_xgboost = predict(xgboost, 
                    newdata=as.matrix(train_input))

rmse_test_xgboost = sqrt(mean((pred_trian_xgboost - train$price)^2)) #0.350234 RMSE test